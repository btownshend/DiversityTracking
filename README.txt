...
- the pool of molecules initially contains a given fraction of molecules which are "good".  Each of the "good" molecules is assumed to initially have a unique sequence, but will later have multiple copies. The distribution of the number of occurences of a sequence is handled empirically using Monte Carlo simulation with 10,000 random sequence numbers at the start, which are then sampled/resampled etc in the same way the mixture is sampled. This sample is used in the computation of the mean number of copies of each sequence.
...
Several step of processing cause the pool of molecules to be sampled, with or without replacement.  For example, gel extraction results in some fraction of the molecules to be recovered, without replacement.  Transcription, however, samples with replacement -- the same molecule may be transcribed multiple times. The sampling steps result in changes to the distribution of "good" sequences. This distribution can be plotted with plotdist(), but is usually just assessed by a single statistic, "kgood", which is the mean number of copies of each good sequence that is present.  Note that kgood will always be >=1.0 since only sequences which are present are considered.

Internally, the class keeps track of the total number of molecules in each class and the good sequence distribution.  In addition, it uses a 'volume' property to be able to track the current volume and concentration of the pool.
